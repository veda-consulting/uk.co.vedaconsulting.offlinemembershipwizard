...
{else}
      ...
      <table class="form-layout-compressed">
        ...
        <tr class="crm-membership-form-block-membership_type_id">
          ...
          <td><!--<span id='mem_type_id'>{$form.membership_type_id.html}</span>-->
            ...
            <span class="description">{ts}Select a price set.{/ts}</span>
          </td>
        </tr>
        ...
        {/if}
          ...
          <tr class="crm-membership-form-block-record_contribution"><td colspan="2">
            <fieldset id="recordContribution" style="display:block;"><legend>{ts}Payment Options{/ts}</legend>
              <table>
                <tr>
                ...
                <tr style="display: none;">
                  ...
                  <td id="payment_processor_td">&nbsp;{$form.payment_processor.html}</td>
                </tr>
                <!-- GK 19092017 - Placeholder to display warning messages -->
                <tr style="display: none;" id="warning_message">
                  <td class="label"><span class="icon inform-icon""></span></td>
                  <td class="messages status no-popup"></td>
                </tr>
              </table>
            </fieldset></td>
          </tr>
  ...
  {if $action neq 8} {* Jscript additions not need for Delete action *}
    ...
    <script type="text/javascript">
      ...
      cj( document ).ready(function() {
        // DS: trigerring buildAMount() on load, supresses the discount
        // The side effect is - in the wizard when you come back to this screen, we have to
        // re-select the price set & re-apply the discount.
        //buildAmount();
        buildPaymentProcessor();
      });
    ...
    var paymentMethodProcessor = {/literal}{$paymentMethodProcessor}{literal};
    //console.log(paymentMethodProcessor);
    // GK 21092017 - WarningMessages
    var warningMessages = {/literal}{$warningMessages}{literal};
    ...
    function buildPaymentProcessor( paymentMethodId ) {
      if ( !paymentMethodId ) {
        paymentMethodId = cj("#payment_instrument_id").val( );
      }

      var processorPerMethod = paymentMethodProcessor[paymentMethodId];
      //console.log(processorPerMethod);

      if(typeof processorPerMethod != 'undefined') {
        removeAllPaymentProcessors();
        cj.each( processorPerMethod, function(index, value){
          var radioOption = '<input value="'+index+'" id="CIVICRM_QFID_'+index+'_payment_processor" name="payment_processor" class="crm-form-radio" type="radio" /><label for="CIVICRM_QFID_'+index+'_payment_processor">'+value+'</label>';
          var radioBtn = cj(radioOption);
          radioBtn.appendTo('#payment_processor_td');
        });
        cj("input:radio[name=payment_processor]").parent().parent().show();
      } else {
        cj("input:radio[name=payment_processor]").parent().parent().hide();
      }

      // GK 19092017 - Display Warning message of selected payment processor
      displayWarningMessage();
    }

    // GK 19092017 - Function to display relevant Warning message of selected payment processor
    function displayWarningMessage() {
      cj("#warning_message").hide();
      cj("input:radio[name=payment_processor]").change(function() {
        var selectedProcessorId = this.value;
        var warningMessage = warningMessages[selectedProcessorId];

        // display warning message, if the selected precessor has any
        if (warningMessage != '') {
          cj("#warning_message .status").html(warningMessage);
          cj("#warning_message").show();
        } else {
          cj("#warning_message").hide();
        }
      });
    ...
    function removeAllPaymentProcessors() {
      /*cj("input:radio[name=payment_processor]").each(function() {
        var radioValue = cj(this).val();
        var radioElement = 'CIVICRM_QFID_'+radioValue+'_payment_processor';
        cj('#'+radioElement).remove();
        cj('label[for='+radioElement+']').remove();
      });*/
      cj('#payment_processor_td').html('&nbsp;');
    }
  ...
  if ( !priceSetId ) {
    priceSetId = cj("#price_set_id").val( );
      ...
      var contactId = cj('[name="contactID"]').val();

      var dataUrl = {/literal}"{crmURL h=0 q='snippet=4'}"{literal} + '&priceSetId=' + priceSetId + '&cid=' + contactId;
